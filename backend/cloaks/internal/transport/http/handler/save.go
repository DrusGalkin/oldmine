...
// Save godoc
// @Summary      Загрузка плаща
// @Description  Загрузка PNG файла плаща
// @Tags         Cloaks
// @Accept       multipart/form-data
// @Produce      json
// @Security     CookieAuth
// @Param        file formData file true "PNG файл плаща"
// @Success      201  {object}  object
// @Success      400  {object}  dto.ErrorResponse
// @Success      401  {object}  dto.ErrorResponse
// @Success      500  {object}  dto.ErrorResponse
// @Router       /api/cloaks/ [post]
func (h *CloaksHandler) Save(ctx fiber.Ctx) error {
	...
	tmpFile, err := os.CreateTemp("", "upload_cloak_*.png")
	if err != nil {
	...
	if bounds.Dx() > 64 || bounds.Dy() > 32 {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"message": "Изображение должно быть размером 64x32 или 22х17 пикселей",
		})
	}
	...
	// Возможно, кто-то захочет сохранить плащ, когда он уже существует, значит нужно понять, хранится ли в бд
